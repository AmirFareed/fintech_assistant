...
def vector_retrieval_enabled() -> bool:
    value = os.getenv("ENABLE_VECTOR_RETRIEVAL")
    if value is None:
        return Config.ENABLE_VECTOR_RETRIEVAL
...
def load_chunks(
    ...
    query = db.table("chunks").select(CHUNK_COLUMNS).order("department_file_id").order("chunk_index")
    if department_id:
