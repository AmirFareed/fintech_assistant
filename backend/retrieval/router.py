...
OTHER_BANK_WORDS = [
    ...
    "meezan", "meezan bank", "bank of khyber", "bank al habib",
    "allied bank", "askari", "faysal", "standard chartered",
...
def detect_intent(query: str) -> str:
    ...
    if len(words) <= 3 and any(greeting in q for greeting in GREETING_WORDS):
        return "greeting"
    for phrase in CONVERSATIONAL_GREETING_PHRASES:
        if phrase not in q:
            continue
        remaining = re.findall(r"[a-z]+|[\u0600-\u06ff]+", q.replace(phrase, " "))
        if all(word in GREETING_FILLER_WORDS for word in remaining):
            return "greeting"
