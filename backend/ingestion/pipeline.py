...
from vectordb.file_store import remove_file, save_file
...
from retrieval.search import vector_retrieval_enabled
...
SERVICE_NAME_ALIASES = {
    ...
    "supported banks psid payment": "Other Banks PSID Payment",
}


def service_slug(service_name: str) -> str:
    """Build a stable slug for an explicitly named service block."""
    slug = re.sub(r"[^a-z0-9]+", "-", (service_name or "").lower()).strip("-")
    return slug or f"service-{uuid.uuid4().hex[:12]}"


def service_keywords(service_name: str) -> list[str]:
    """Create useful routing phrases for a newly discovered payment service."""
    normalized = normalize_service_name(service_name) or service_name
    channel_name = re.sub(r"(?i)\s+psid\s+payment\s*$", "", normalized).strip()
    candidates = [
        channel_name,
        f"pay via {channel_name}",
        f"{channel_name} psid",
        normalized,
    ]
    return list(dict.fromkeys(value.lower() for value in candidates if value))
...
def upload_file_to_storage(local_file_path: str, storage_name: str, original_file_name: str):
    get_content_type(original_file_name)  # rejects unsupported file types
    save_file(local_file_path, storage_name)


def create_department_file_record(department_id: str, file_name: str, storage_path: str):
    result = db.table("department_files").insert({
...
def get_service_by_name(department_id: str, service_name: str):
    ...
    generic_tokens = {"bank", "banks", "psid", "payment", "payments", "service"}
    service_name_tokens = {
...
def get_or_create_service(department_id: str, service_name: str):
    """Resolve a service block, creating a routable service when necessary."""
    normalized_name = normalize_service_name(service_name)
    if not normalized_name:
        return None

    service = get_service_by_name(department_id, normalized_name)
    if service:
        return service

    slug = service_slug(normalized_name)
    by_slug = (
        db.table("services")
        .select("*")
        .eq("department_id", department_id)
    ...
    channel_name = re.sub(r"(?i)\s+psid\s+payment\s*$", "", normalized_name).strip()
    created = db.table("services").insert({
        "department_id": department_id,
        "service_name": normalized_name,
        "service_slug": slug,
        "summary": f"How to pay an official PSID through {channel_name}.",
        "keywords": service_keywords(normalized_name),
        "is_active": True,
    }).execute()
...
def rollback_failed_ingestion(department_file: dict | None, storage_path: str) -> None:
    """Best-effort cleanup so a failed upload cannot leave a partial index."""
    if department_file and department_file.get("id"):
        file_id = department_file["id"]
        try:
    ...
    try:
        ...
        if service_blocks:
            ...
            for block in service_blocks:
                ...
                service = get_or_create_service(department_id, raw_service_name)
                if service:
        ...
        rollback_failed_ingestion(department_file, unique_name)
        raise
