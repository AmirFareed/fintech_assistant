"""
Ingests all 4 FinTech RAG files into the database under the Digital Payments department.
...
def main():
    res = db.table("departments").select("id").eq("slug", "digital-payments").single().execute()
    if not res.data:
