...
from llm.chatbot import handle_chat_query, debug_chat_query, MAX_QUERY_LENGTH
...
# Slow down password guessing on the (publicly reachable) admin login.
app.view_functions["admin.login"] = limiter.limit("10 per minute")(app.view_functions["admin.login"])
...
def format_backend_error(exc):
    if isinstance(exc, OperationalError):
        return "The knowledge base is currently unavailable. Check the database connection and try again."
    return str(exc)
...
@app.route("/")
def index():
    return jsonify({"service": "fintech-assistant-api", "version": "2.0"})


@app.route("/health")
def health():
    status = {"status": "ok", "database": "ok", "version": "2.0"}
    try:
