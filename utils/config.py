...
class Config:
    ...
    SUPABASE_URL = os.getenv("SUPABASE_URL", _value(_CONFIG, "vectordb", "url", None))
    SUPABASE_KEY = os.getenv("SUPABASE_KEY", _value(_CONFIG, "vectordb", "key", None))
    SUPABASE_BUCKET = os.getenv("SUPABASE_BUCKET", _value(_CONFIG, "vectordb", "bucket", "rag-files"))
    SUPABASE_TIMEOUT_SECONDS = float(
        os.getenv("SUPABASE_TIMEOUT_SECONDS", _value(_CONFIG, "vectordb", "timeout_seconds", 5))
    )
