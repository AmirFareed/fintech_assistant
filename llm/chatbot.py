...
from retrieval.router import detect_intent, find_best_service
from retrieval.search import search_chunks, search_chunks_debug
...
SEARCH_MATCH_COUNT = Config.RETRIEVAL_MATCH_COUNT
...
def resolve_service(user_query: str, intent: str) -> dict | None:
    # Named banks share the broad intent, but must resolve to their own service
    # before the generic Other Banks fallback is considered.
    if intent == "other_banks_payment":
        best_service, _ = find_best_service(user_query)
        if best_service and best_service.get("service_name") != "Other Banks PSID Payment":
            return best_service
...
def handle_chat_query(
    ...
    # General conversation is answered directly instead of being forced through
    # the payment-only knowledge base and its out-of-scope fallback.
    if intent == "general_help":
        answer, usage = generate_general_chat_response(
        ...
        if usage.get("total_tokens", 0) > 0 and session_id:
            from llm.client import GROQ_MODEL
